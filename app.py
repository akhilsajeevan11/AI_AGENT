...
try:
    from main_crew import initialize_llm 
except ImportError: 
    print("Warning: Could not import initialize_llm from main_crew.py. Using fallback.")
    ...
    def initialize_llm():
        ...
        llm = None
        if llm_provider == "openai" and openai_api_key: llm = ChatOpenAI(model_name=openai_model_name, api_key=openai_api_key, temperature=0.0)
        elif llm_provider == "groq" and groq_api_key: llm = ChatGroq(model_name=groq_model_name, groq_api_key=groq_api_key, temperature=0.0)
        if not llm and openai_api_key: llm = ChatOpenAI(model_name=openai_model_name, api_key=openai_api_key, temperature=0.0)
        if not llm: print("LLM could not be initialized from app.py fallback. Check API keys in .env.")
        return llm
...
try:
    llm = initialize_llm()
    if llm is None:
        app.logger.error("LLM could not be initialized. Ensure API keys are set in .env file.")
    else:
        app.logger.info(f"LLM initialized successfully using provider: {os.getenv('LLM_PROVIDER', 'openai')}")
        ...
        app.logger.info("CrewAI Agents initialized.")
except Exception as e:
    app.logger.error(f"Critical error during LLM or Agent initialization: {e}", exc_info=True)
...
def register_patient_page(): 
    return render_template('register_patient.html', active_page='register_patient', patient_data={}) # Pass empty patient_data for fresh form
...
def run_crew_in_thread(crew_instance, task_run_id, app_context=None):
    ...
    try:
        with current_app_context: # Ensure thread has app context if needed by any part of crew/tools
...
def submit_patient_registration():
    ...
    required_fields = ["first_name", "last_name", "phone_number", "email"] # Date of birth, gender, address are also good to have
    ...
    try:
        ...
        if run_async:
            ...
            # Proper async handling needs a task queue (Celery/RQ) and robust result management.
            ...
            # TODO: ASYNCHRONOUS PROCESSING: In a production environment, this CrewAI task
            # should be submitted to a background task queue. For now, execution is synchronous.
            raw_result = crew.kickoff()
            processed_result = process_crew_result(raw_result)
            # Determine flash category based on result (simple check)
...
def fetch_doctor_availability():
    ...
    if not llm or not medical_records_agent:
        flash("ERROR: AI components not initialized.", "danger")
        return render_template('view_doctors.html', doctor_id_for_avail=doctor_id_str, start_date_for_avail=start_date, end_date_for_avail=end_date, active_page='view_doctors')
    ...
    try:
        ...
        raw_result = crew.kickoff()
        processed_result = process_crew_result(raw_result)
        return render_template('view_doctors.html', availability_data=processed_result, doctor_id_for_avail=doctor_id_str, start_date_for_avail=start_date, end_date_for_avail=end_date, active_page='view_doctors')
    except ValueError:
...
def fetch_patient_appointments():
    ...
    if not llm or not medical_records_agent:
        flash("ERROR: AI components not initialized.", "danger")
        return render_template('view_appointments.html', patient_phone=phone_number, active_page='view_my_appointments')
    ...
    try:
        ...
        raw_result = crew.kickoff()
        processed_result = process_crew_result(raw_result)
        return render_template('view_appointments.html', appointments_data=processed_result, patient_phone=phone_number, active_page='view_my_appointments')
    except Exception as e:
...
def submit_schedule_appointment():
    ...
    try:
        ...
        task = Task(
            description=task_def['description'].format(appointment_details_json=json.dumps(form_data)),
            expected_output=task_def['expected_output'],
        ...
        raw_result = crew.kickoff()
        processed_result = process_crew_result(raw_result)
        flash("Appointment scheduling processed.", "info") # General flash, result shown below
